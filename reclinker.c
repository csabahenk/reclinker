...
int removelink()
{
	...
	removal:
	  ...
	  {
	  	if (verbosity >= 1) printf("%s%s removed\n",whereorep,where->strmid);
	  }
	  else
	  {
...
int linker()
{
	...
	if(symlink(prefix->str,where->str) == 0)
	{
		if(verbosity >= 1)
		{
			...
			if (verbosity >= 2) printf(": symlink created");
			putchar('\n');
		}
...
int test()
{
	...
	/* "what" is a dir */
		...
		{
			if(verbosity >= 2) printf("%s%s/ is OK\n",whereorep,where->strmid);
		}
		else
		{
...
void fetchdir(struct pathnode *root)
{
	DIR *dir;
	struct dirent *entry;
	struct pathnode *pn;
...
void nextinpath(struct pathnode *root)
{
...
void reclinker()
{
	...
	while(pn = pn->next)
	{
		...
		what = pn->name;
		if(strcmp(what,".") == 0 || strcmp(what,"..") == 0) continue;

		postgrowth = strlen(what) + 1;
		appendasadir(where,what);
		if(deletemode != 1){ appendasadir(prefix,what); }
	...
	if(deletemode != 1){ backabit(prefix,postgrowth); }
	}
...
#define initprefix \
	...
	if(!strsubtest(prefix->str,"../")) forceproperprefix = 1
